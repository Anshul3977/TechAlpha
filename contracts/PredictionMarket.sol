...
contract PredictionMarket is Ownable {
    ...
    struct Bet {
        ...
        uint256 amount;         // Amount bet in ZETA
    ...
    // Sybil resistance: fee per bet in ZETA (0.01 ZETA)
    uint256 public constant BET_FEE = 1e16;
    ...
    // Create a new market (only owner)
    ...
    function getOutcomesLength(uint256 _marketId) external view returns (uint256) {
        ...
        return markets[_marketId].outcomes.length;
    }
    ...
    function placeBet(uint256 _marketId, uint256 _outcomeIndex, uint256 _amount) external {
        ...
        require(_outcomeIndex < markets[_marketId].outcomes.length, "Invalid outcome index");

        // Collect Sybil resistance fee
        zetaToken.transferFrom(msg.sender, address(this), BET_FEE);

        // Transfer bet amount
        zetaToken.transferFrom(msg.sender, address(this), _amount);
    ...
    function resolveMarket(uint256 _marketId, uint256 _winningOutcome) external onlyOwner {
        ...
        require(!markets[_marketId].resolved, "Market already resolved");
        require(_winningOutcome < markets[_marketId].outcomes.length, "Invalid winning outcome");
    ...
    function distributeRewards(uint256 _marketId, uint256 _winningOutcome) private {
        ...
        uint256 totalPool = 0;    // Total ZETA bet on the market
